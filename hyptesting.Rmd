...
- What assumptions?
  ...
  - No uncontrolled sources of bias (confounding, p-hacking, programming error, etc)
...
- A very small P-value does not tell us which model assumption is incorrect:
  ...
  - But also that some auxiliary assumption is not true (e.g. sampling not random, p-hacking...)
...
p2 <- ggplot(samp) +
  ...
  labs(title = "Sampling 500 individuals from each group and making t-test",
       y = "count") +
  annotate("text", x = 75, y = 80, hjust = 0, size = 6,
           label = paste0("p = ", round(tt$p.value, 3)))
...
- My suggestion: avoid significant/not significant (perhaps p-values too)

- Beyond significance, look at *effect sizes* and their uncertainty.
